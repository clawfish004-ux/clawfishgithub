...
# PIL version error fix
if not hasattr(Image, 'ANTIALIAS'):
    # အကယ်၍ ANTIALIAS မရှိရင် LANCZOS ကို သုံးဖို့ သတ်မှတ်ပေးလိုက်တာပါ
...
async def make_light_video():
    ...
    urls = get_pexels_image_urls("korean idols stage")
    local_images = []
    ...
    for img_path in local_images:
        # Resize logic ကို ImageMagick မလိုတဲ့ ရိုးရိုးနည်းနဲ့ပဲ သုံးထားပါတယ်
    ...
    # Bitrate ကို လျှော့ချထားလို့ GitHub မှာ မြန်မြန်ပြီးမှာပါ
...
async def main():
    print("🚀 Starting Production (PIL Error Fixed Version)...")
    try:
        ...
        if result:
            ...
            with open(path, "rb") as v:
                requests.post(url, files={"video": v}, data={"chat_id": TELEGRAM_CHAT_ID, "caption": "🎬 1-Minute Daily K-News"})
